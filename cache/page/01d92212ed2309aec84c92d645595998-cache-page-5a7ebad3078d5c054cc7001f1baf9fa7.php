<?php die("Access Denied"); ?>#x#a:3:{s:4:"body";s:77855:" 
...
<meta name="viewport" content="width=device-width" />
		  ...
		  <base href="http://www.inmotionhosting.com/support/website/php/choosing-the-best-php-handler" />
  <meta http-equiv="content-type" content="text/html; charset=utf-8" />
  <meta http-equiv="last-modified" content="2014-10-08T17:23:22-07:00" />
  <meta name="keywords" content="php handler handlers method suphp dso fcgi fastcgi cgi mod_suphp mod_php mod_fcgid mod_cgi" />
  <meta name="author" content="Jacob Nicholson" />
  <meta name="description" content="A PHP handler defines the method that Apache uses to communicate back and forth with your PHP scripts. Knowing the differences between PHP handlers can help ensure you're using the best one for your PHP environment. The main 4 are suPHP, DSO, FastCGI, and CGI." />
  <title>Choosing the best PHP handler - InMotion Hosting</title>
  <link href="http://www.inmotionhosting.com/support/website/php/choosing-the-best-php-handler" rel="canonical" />
...
<script type="text/javascript">
  ...
  (function() {
	...
	<!-- cid:00d143e65fcf7df79afe9a65f100d0a9 -->
...
<script type="text/javascript">
                ...
                window.open('https://www.inmotionhosting.com/support/launch-chat/?rdid=2324923566971164486&url=' + escape(document.location.href), 'Chat8718689652968401074', 'toolbar=0,scrollbars=1,location=0,statusbar=0,menubar=0,resizable=1,width=640,height=650');
                        ...
                        _gaq.push(['_setAccount', 'UA-1501988-22']);
                        _gaq.push(['_trackEvent', 'chat-by-ip', 'launch-by-ip', '80.58.146.197']);
...
<div class='container'>
	<div class='main-left-sc' name='sc_left' id='sc_left' style='max-width:705px;'>
		...
		<div class="page-header">
						<h1>
															Choosing the best PHP handler									</h1>
					...
					<div style='font-size:12px; color:#aaa; margin-bottom:-10px; padding-top:5px;'>
							...
							<div style='float:left; margin-right:20px;'><span style='color:#606060;'>Views:</span> <span name='page_views' id='page_views'>45,068</span></div>
							<div style='float:left; margin-right:20px;' title='Originally published: Oct 24, 2012'><span style='color:#606060;'>Published:</span> Oct 8, 2014</div>
							...
							<div style='clear:both;'></div>
								...
								<p>In this article we'll discuss the different PHP handler options that you have to choose from on your <a href="http://www.inmotionhosting.com/vps-hosting">VPS (Virtual Private Server)</a> or <a href="http://www.inmotionhosting.com/dedicated-servers">dedicated server</a> running cPanel. A PHP handler defines the method that Apache uses to communicate back and forth with your PHP scripts. Knowing the differences between PHP handlers can help ensure you're using the best one for your PHP environment.</p>
<p>Choosing the best PHP handler for your specific needs can go a long way in ensuring an efficient and secure hosting experience for all of your websites. One PHP handler is not necessarily better than the others in every situation, so understanding the basics of what each PHP handler has to offer can help you figure out which one might be best in your specific case.</p>
<h2>Available PHP handlers</h2>
<table class="article_table">
<tbody>
<tr><th style="width: 35%;">suPHP (Single user PHP)</th>
<td style="padding: 15px;">On InMotion Hosting's servers, suPHP is the default PHP handler that is used. suPHP runs as a CGI module and its major benefits are stronger security and easy permissions.</td>
</tr>
<tr><th>DSO (Dynamic Shared Object)</th>
<td style="padding: 15px;">Run as an Apache module, DSO also referred to as mod_php, is one of the oldest PHP handlers and its major benefits are speed and the ability to use opcode caching.</td>
</tr>
<tr><th>FCGI (FastCGI)</th>
<td style="padding: 15px;">Run as a CGI module, FCGI also referred to as mod_fcgid, is one of the newest PHP handlers and its major benefits are CPU performance and the ability to use opcode caching.</td>
</tr>
<tr><th>CGI (Common Gateway Interface)</th>
<td style="padding: 15px;">Run as a CGI module, the CGI PHP handler supports some extra configurations, but lacks in both speed and security and is generally not recommended to use.</td>
</tr>
</tbody>
</table>
<h3>suPHP - Single user PHP (mod_suphp)</h3>
<p>Using the suPHP PHP handler is among the most flexible and secure ways of serving PHP requests. With the suPHP handler selected along with using the suEXEC option, all PHP scripts will be executed as the user that owns the script, instead of running as Apache's "nobody" user. For every PHP request that is being processed a separate PHP process will be spawned, so it makes it very easy to see and track if one particular user on the server is running a lot of PHP scripts, or which users are causing more CPU intensive PHP script executions.</p>
<p>The main benefit of using the suPHP handler is that it isolates one user on the server from the others. So if one account was compromised because of an exploit in one of your PHP scripts, the attacker would only be able to view or modify files owned by that particular user. It's also among the easiest to use with CMS (Content Management Systems) such as Wordpress or Joomla. As typically these applications require the ability to create files on the server, and being that all of your files are owned by just one user, <a href="/support/website/how-to/change-file-permissions">permission management</a> is easy to configure.</p>
<p>The main disadvantage of using the suPHP handler is that your PHP websites could be slower due to the additional overhead of having to run a separate PHP process per request. Also if your server receives a high volume of PHP requests in a short period of time, this could lead to a heavy load on your server from all of the concurrent PHP processes. Finally you won't be able to change PHP directives directly from a .htaccess file, you would instead need to make these changes in a php.ini file. For more information on this, you can read our guide <a href="/support/website/general-server-setup/recursive-php-ini">how can I make my php.ini file recursive?</a></p>
<p>You might choose suPHP as your PHP handler if you have multiple users on your server, you don't want to worry about setting permissions, and you aren't having any performance issues with the PHP scripts that you currently run.</p>
<h3>DSO - Dynamic Shared Object (mod_php)</h3>
<p>Using the DSO PHP handler is the fastest way to serve PHP requests. The DSO handler is an older PHP handler that runs PHP as an Apache module. All PHP scripts are executed as the Apache user "nobody", which allows for faster execution, but can be more of a headache configuring permissions for.</p>
<p>The main benefit of using the DSO handler is that it doesn't have much overhead, so it's fast out of the box. You can also use PHP opcode caching along with DSO to speed up your PHP requests even further. Additionally you can set PHP directives directly via .htaccess files to control certain functionality of PHP.</p>
<p>The main disadvantage of using the DSO handler is that all PHP scripts will need to be owned and excuted by Apache's "nobody" user. This means that PHP executions can't easily be tracked on a per user basis since they're all run from the one web server user. Also when you use a CMS (Content Management System) such as WordPress or Joomla that requires the ability of these applications to write or modify files, the proper permissions must be set beforehand to allow this to happen. Each time a new file is created by a PHP script, it will be owned by the "nobody" user and can lead to further permission issues.</p>
<p>Another major drawback of the DSO handler is security. Because all of your users on the server are having their PHP scripts executed by the same "nobody" user, if one of your sites is exploited due to a flaw in one of your PHP scripts, the attacker could potentially look at or modify files outside of that user's directory that had the PHP script that was exploitable. So when using the DSO handler it's always very important to keep all of your PHP software up to date with any security patches so they are not exploited.</p>
<p>You might choose DSO as your PHP handler if you only have one user and your primary concern is speed and performance.</p>
<h3>FCGI - FastCGI (mod_fcgid)</h3>
<p>Using the FastCGI PHP handler is a faster way to serve PHP requests than using suPHP but typically not as fast as using DSO. FastCGI is good at reducing CPU usage by leveraging your server's available RAM to keep PHP scripts in memory instead of having to start up a separate PHP process for each and every PHP request.</p>
<p>The main benefit of using the FastCGI handler is that you can use suEXEC just like with suPHP to allow PHP scripts to be executed by the actual user of the PHP script instead of using the Apache "nobody" user. FastCGI also doesn't require a single PHP process execution per request like suPHP does, so it can be much faster and reduces CPU usage by holding PHP scripts in memory.</p>
<p>The main disadvantage of the FastCGI handler is that it can be very memory intensive. This is because FastCGI keeps PHP sessions opened in the background in memory for quicker access and the ability to use PHP opcode caching can add to the memory usage as well. Additionally FastCGI can encounter an array of errors depending on how your PHP scripts are coded. This can typically require a day or two of setting tweaks, specific to what your PHP scripts are trying to do.</p>
<p>You might choose FastCGI as your PHP handler if you're experiencing slow or excessive PHP executions when using the suPHP handler, and you have available memory to spare on your server.</p>
<h3>CGI - Common Gateway Interface (mod_cgi)</h3>
<p>Using the CGI PHP handler is not something widely used anymore due to the other PHP handlers benefits. Like suPHP and FastCGI the CGI handler can use suEXEC so that PHP executions are run by the file owner of a PHP script rather than the Apache "nobody" user.</p>
<p>The main benefit of using the CGI handler is that it is very configurable, and supports using suEXEC for making permissions less of a headache.</p>
<p>The main disadvantage of the CGI handler is that it is the slowest handler. The CGI handler is typically not used much anymore.</p>
<p>You might choose CGI as your PHP handler if for some reason suPHP, DSO, or FastCGI wasn't available on your server.</p>
<h2>Conclusion</h2>
<p>You should now have a decent understanding of the different PHP handlers available to use. Choosing which PHP handler is best for your particular PHP environment can be tricky at times, and generally it's not recommened to change them if you're not having any performance or security issues with your default setup.</p>
<p>If you'd like a recommendation on which PHP handler might be best in your particluar case we would recommend sending an e-mail to (<a href="mailto:support@inmotionhosting.com">support@inmotionhosting.com</a>) and our system administration team can review your current server configuration for you.</p> 
...
<div style='display:inline-block;'>
	<iframe src='//www.facebook.com/plugins/like.php?href=http://www.inmotionhosting.com/support/website/php/choosing-the-best-php-handler&amp;send=false&amp;layout=button_count&amp;width=100&amp;show_faces=false&amp;font=verdana&amp;colorscheme=light&amp;action=like&amp;height=21&amp;appId=209145459223160' scrolling='no' frameborder='0' style='border:none; overflow:hidden; width:100px; height:21px;' allowTransparency='true'></iframe>
</div>

	                        
<div style='display:inline-block;'>
	<div class='g-plusone' data-size='medium' data-href='http://www.inmotionhosting.com/support/website/php/choosing-the-best-php-handler'></div>
	<script type='text/javascript'>
	  (function() {
	...
	<div name='sc_right' id='sc_right'>
		...
		<input type="hidden" name="return" value="aW5kZXgucGhwP29wdGlvbj1jb21fY29udGVudCZjYXRpZD00MTImaWQ9NDUzJnZpZXc9YXJ0aWNsZSZJdGVtaWQ9MTAx" />
			...
			<table style='width:100%; border-collapse:separate; border:1px solid #aaa;' class='sc_rc'>
				<tr>
					<td valign='top' style='width:100px; padding:5px; border-right:1px solid #aaa;background-image: -webkit-linear-gradient(top,#e3e3e3,#efefef);background-image: linear-gradient(to bottom, #E3E3E3, #EFEFEF); border-bottom-left-radius: 5px; border-top-left-radius: 5px;'>
						<img src='//www.gravatar.com/avatar/b138a6f826edbc71af201992aa812efe?s=90' class='sc_rc' style='max-width:90px; margin-right:15px;display:block;' />
						<div><a style='font-size:16px;' href='//www.gravatar.com/b138a6f826edbc71af201992aa812efe'>Santiago Gonzalez</a></div>
						<div></div>
						<div></div>
					...
					<td valign='top' style='padding:5px; background:#fff; border-bottom-right-radius: 5px; border-top-right-radius: 5px;'>
						<div style='float:right;margin:0px 0px 15px 15px;font-size:12px;color:#aaa'>2015-04-14 11:09 am</div>
				                <div><p>Great articlet, Thanks for such an easy and clear explanation.</p>
<p>&nbsp;</p>
<p>&nbsp;</p></div>
						<div style='text-align:right; clear:both;'><a href='#post_a_comment' class='btn btn-primary' style='text-transform:none;' onClick="jQuery('#new_comment_parent_id').val(22641);jQuery('html, body').animate({scrollTop: jQuery('#main_comment_submission_div').offset().top}, 1500);return false;" >Reply</a></div>
		...
		<a name='comment-23624'></a>
		<div  style='margin:40px 0px 0px 0px;'>
			
			<table style='width:100%; border-collapse:separate; border:1px solid #aaa;' class='sc_rc'>
				<tr>
					<td valign='top' style='width:100px; padding:5px; border-right:1px solid #aaa;background-image: -webkit-linear-gradient(top,#e3e3e3,#efefef);background-image: linear-gradient(to bottom, #E3E3E3, #EFEFEF); border-bottom-left-radius: 5px; border-top-left-radius: 5px;'>
						<img src='//www.gravatar.com/avatar/836531d2d76430d907b12002047dbcea?s=90' class='sc_rc' style='max-width:90px; margin-right:15px;display:block;' />
						<div><a style='font-size:16px;' href='//www.gravatar.com/836531d2d76430d907b12002047dbcea'>Pallabi Saha</a></div>
						<div></div>
						<div></div>
						<div><span title='Posted anonymously - anonymous users receive no points'>n/a <a href='/support/team/points' target='_blank'>Points</a></span></div>
					...
					<td valign='top' style='padding:5px; background:#fff; border-bottom-right-radius: 5px; border-top-right-radius: 5px;'>
						<div style='float:right;margin:0px 0px 15px 15px;font-size:12px;color:#aaa'>2015-05-22 8:33 am</div>
				                <div><p>Just a single read through and the concept becomes clear. Thanks for such a simple and clearly conceptualized article! :)</p></div>
						<div style='text-align:right; clear:both;'><a href='#post_a_comment' class='btn btn-primary' style='text-transform:none;' onClick="jQuery('#new_comment_parent_id').val(23624);jQuery('html, body').animate({scrollTop: jQuery('#main_comment_submission_div').offset().top}, 1500);return false;" >Reply</a></div>
		...
		<a name='comment-24673'></a>
		<div  style='margin:40px 0px 0px 0px;'>
			
			<table style='width:100%; border-collapse:separate; border:1px solid #aaa;' class='sc_rc'>
				<tr>
					<td valign='top' style='width:100px; padding:5px; border-right:1px solid #aaa;background-image: -webkit-linear-gradient(top,#e3e3e3,#efefef);background-image: linear-gradient(to bottom, #E3E3E3, #EFEFEF); border-bottom-left-radius: 5px; border-top-left-radius: 5px;'>
						<img src='//www.gravatar.com/avatar/47defd024ce149c8bb31f33c2aa36d3c?s=90' class='sc_rc' style='max-width:90px; margin-right:15px;display:block;' />
						<div><a style='font-size:16px;' href='//www.gravatar.com/47defd024ce149c8bb31f33c2aa36d3c'>Hemm Rojd</a></div>
						<div></div>
						<div></div>
						<div><span title='Posted anonymously - anonymous users receive no points'>n/a <a href='/support/team/points' target='_blank'>Points</a></span></div>
					...
					<td valign='top' style='padding:5px; background:#fff; border-bottom-right-radius: 5px; border-top-right-radius: 5px;'>
						<div style='float:right;margin:0px 0px 15px 15px;font-size:12px;color:#aaa'>2015-07-06 9:50 pm</div>
				                <div><p>.htaccess doesn't work with suPHP? That's news to me. It sure seems to work on my server.</p></div>
						<div style='text-align:right; clear:both;'><a href='#post_a_comment' class='btn btn-primary' style='text-transform:none;' onClick="jQuery('#new_comment_parent_id').val(24673);jQuery('html, body').animate({scrollTop: jQuery('#main_comment_submission_div').offset().top}, 1500);return false;" >Reply</a></div>
		...
		<div  style='margin:2px 0px 0px 40px;'>
			...
			<table style='width:100%; border-collapse:separate; border:1px solid #aaa;' class='sc_rc'>
				<tr>
					...
					<td valign='top' style='padding:5px; background:#fff; border-bottom-right-radius: 5px; border-top-right-radius: 5px;'>
						<div style='float:right;margin:0px 0px 15px 15px;font-size:12px;color:#aaa'>2015-07-06 9:53 pm</div>
				                <div>Hello Hemm,<br />
<br />
.htaccess works fine with suPHP, but what it says is that you cannot change php directives with the .htaccess file. Those need to be done within the php.ini file. You will get a 500 error if you attempt to change php directives directly within the htaccess file.<br />
<br />
Kindest Regards,<br />
Scott M</div>
						<div style='text-align:right; clear:both;'><a href='#post_a_comment' class='btn btn-primary' style='text-transform:none;' onClick="jQuery('#new_comment_parent_id').val(24674);jQuery('html, body').animate({scrollTop: jQuery('#main_comment_submission_div').offset().top}, 1500);return false;" >Reply</a></div>
		...
		<a name='comment-33056'></a>
		<div  style='margin:2px 0px 0px 80px;'>
			
			<table style='width:100%; border-collapse:separate; border:1px solid #aaa;' class='sc_rc'>
				<tr>
					<td valign='top' style='width:100px; padding:5px; border-right:1px solid #aaa;background-image: -webkit-linear-gradient(top,#e3e3e3,#efefef);background-image: linear-gradient(to bottom, #E3E3E3, #EFEFEF); border-bottom-left-radius: 5px; border-top-left-radius: 5px;'>
						<img src='//www.gravatar.com/avatar/5675114b7e0b3c007e9bc84b02881450?s=90' class='sc_rc' style='max-width:90px; margin-right:15px;display:block;' />
						<div><a style='font-size:16px;' href='//www.gravatar.com/5675114b7e0b3c007e9bc84b02881450'>Scott</a></div>
						<div></div>
						<div></div>
						<div><span title='Posted anonymously - anonymous users receive no points'>n/a <a href='/support/team/points' target='_blank'>Points</a></span></div>
					...
					<td valign='top' style='padding:5px; background:#fff; border-bottom-right-radius: 5px; border-top-right-radius: 5px;'>
						<div style='float:right;margin:0px 0px 15px 15px;font-size:12px;color:#aaa'>2016-05-30 8:54 am</div>
				                <div><p>Scott,</p>
<p>I would say installing htscanner is an option here:</p>
<p>https://pecl.php.net/package/htscanner</p>
<p>"Allow one to use htaccess-like file to configure PHP</p>
<p>per directory, just like apache's htaccess. It is especially useful with fastcgi (ISS5/6/7, lighttpd, etc.)."</p></div>
						<div style='text-align:right; clear:both;'><a href='#post_a_comment' class='btn btn-primary' style='text-transform:none;' onClick="jQuery('#new_comment_parent_id').val(33056);jQuery('html, body').animate({scrollTop: jQuery('#main_comment_submission_div').offset().top}, 1500);return false;" >Reply</a></div>
		...
		<a name='comment-31549'></a>
		<div  style='margin:40px 0px 0px 0px;'>
			
			<table style='width:100%; border-collapse:separate; border:1px solid #aaa;' class='sc_rc'>
				<tr>
					<td valign='top' style='width:100px; padding:5px; border-right:1px solid #aaa;background-image: -webkit-linear-gradient(top,#e3e3e3,#efefef);background-image: linear-gradient(to bottom, #E3E3E3, #EFEFEF); border-bottom-left-radius: 5px; border-top-left-radius: 5px;'>
						<img src='//www.gravatar.com/avatar/c6a979f6b2c031498b20e056a11c3501?s=90' class='sc_rc' style='max-width:90px; margin-right:15px;display:block;' />
						<div><a style='font-size:16px;' href='//www.gravatar.com/c6a979f6b2c031498b20e056a11c3501'>popnoart</a></div>
						<div></div>
						<div></div>
						<div><span title='Posted anonymously - anonymous users receive no points'>n/a <a href='/support/team/points' target='_blank'>Points</a></span></div>
					...
					<td valign='top' style='padding:5px; background:#fff; border-bottom-right-radius: 5px; border-top-right-radius: 5px;'>
						<div style='float:right;margin:0px 0px 15px 15px;font-size:12px;color:#aaa'>2016-03-30 3:53 am</div>
				                <div><p>Great article, so easy to understand, thanks a lot!</p></div>
						<div style='text-align:right; clear:both;'><a href='#post_a_comment' class='btn btn-primary' style='text-transform:none;' onClick="jQuery('#new_comment_parent_id').val(31549);jQuery('html, body').animate({scrollTop: jQuery('#main_comment_submission_div').offset().top}, 1500);return false;" >Reply</a></div>
