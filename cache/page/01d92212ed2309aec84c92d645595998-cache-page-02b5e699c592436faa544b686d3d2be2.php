...
<meta name="viewport" content="width=device-width" />
		  ...
		  <base href="http://www.inmotionhosting.com/support/website/modsecurity/disable-modsecurity-for-a-domain" />
  <meta http-equiv="content-type" content="text/html; charset=utf-8" />
  <meta http-equiv="last-modified" content="2013-08-15T18:56:57-07:00" />
  <meta name="keywords" content="disable modsecurity mod_security domain" />
  ...
  <link href="http://www.inmotionhosting.com/support/website/modsecurity/disable-modsecurity-for-a-domain" rel="canonical" />
...
<script type="text/javascript">
  ...
  (function() {
	...
	<!-- cid:c27014118495074b9da8168ca3581362 -->
...
<div class='container'>
	<div class='main-left-sc' name='sc_left' id='sc_left' style='max-width:705px;'>
		...
		<div class="page-header">
						<h1>
															Disable ModSecurity for a domain									</h1>
					...
					<div style='font-size:12px; color:#aaa; margin-bottom:-10px; padding-top:5px;'>
							...
							<div style='float:left; margin-right:20px;'><span style='color:#606060;'>Views:</span> <span name='page_views' id='page_views'>18,479</span></div>
...
<p>If you're on a shared server and experiencing problem with 406 errors caused by ModSecurity you can follow my guide on how to <a href="/support/website/website-troubleshooting/disable-mod-security-via-modsec-manager" target="_blank">disable ModSecurity via cPanel Modsec Manager</a>.</p>
<p class="alert">In order to follow along with this guide, you'll need to have <a href="/support/website/general-server-setup/root-access-faq" target="_blank">root access</a> to your VPS or dedicated server so that you can create the required ModSecurity file on the server, and modify the Apache configuration file.</p>
...
<li>Make a backup of your Apache configuration with the following command:<br /><br /> <span class="code_block">cp -frp /usr/local/apache/conf/httpd.conf{,-BAK}</span><br /><br /></li>
<li>Edit your Apache <strong>httpd.conf</strong> file with the <strong>vim</strong> editor with this command:<br /><br /> <span class="code_block">vim /usr/local/apache/conf/httpd.conf</span><br /><br /></li>
<li>Once in <strong>vim</strong> you can type in a forward slash <strong>/</strong> to enter search mode, you can then enter in the name of the domain you'd like to disable ModSecurity for and hit <strong>Enter</strong>&nbsp;to be dropped to that line.
<p>You should be at a <strong>&lt;VirtualHost</strong> entry that looks something like this:</p>
  <p class="code_block" style="width: 500px;">&lt;VirtualHost 123.123.123.123:80&gt;<br /> ServerName example.com<br /> ServerAlias www.example.com<br /> DocumentRoot /home/userna5/public_html<br /> ServerAdmin webmaster@example.com<br /> UseCanonicalName Off<br /> CustomLog /usr/local/apache/domlogs/example.com combined<br /> CustomLog /usr/local/apache/domlogs/example.com-bytes_log "%{%s}t %I .\n%{%s}t %O ."<br /> ## User userna5 # Needed for Cpanel::ApacheConf<br /> &lt;IfModule mod_suphp.c&gt;<br /> suPHP_UserGroup userna5 userna5<br /> &lt;/IfModule&gt;<br /> &lt;IfModule !mod_disable_suexec.c&gt;<br /> &lt;IfModule !mod_ruid2.c&gt;<br /> SuexecUserGroup userna5 userna5<br /> &lt;/IfModule&gt;<br /> &lt;/IfModule&gt;<br /> &lt;IfModule mod_ruid2.c&gt;<br /> RUidGid userna5 userna5<br /> &lt;/IfModule&gt;<br /> ScriptAlias /cgi-bin/ /home/userna5/public_html/cgi-bin/<br /><br /> # To customize this VirtualHost use an include file at the following location<br /> <strong style="color: red;"># Include "/usr/local/apache/conf/userdata/std/2/userna5/example.com/*.conf"</strong><br /><br /> &lt;/VirtualHost&gt;</p>
<p>You'll want to un-comment the <strong>Include</strong> line by removing the pound <strong>#</strong> symbol from the beginning of the line at the bottom. You can do this by navigating with the arrow keys till your cursor is over the <strong>#</strong> symbol, then just hit <strong>Delete</strong> on your keyboard:</p>
  <p class="code_block" style="width: 500px;"><strong style="color: red;">#</strong> Include "/usr/local/apache/conf/userdata/std/2/userna5/example.com/*.conf"</p>
<p>So it should end up looking like this:</p>
<p class="code_block" style="width: 500px;">Include "/usr/local/apache/conf/userdata/std/2/userna5/example.com/*.conf"</p>
<p>Now to save the file, simply hold down <strong>Shift</strong> and hit <strong>ZZ</strong> on the keyboard, you should get this confirmation:</p>
<p class="code_block" style="width: 500px;">"/usr/local/apache/conf/httpd.conf" 785L, 30554C written</p>
</li>
  <li>Finally run the cPanel Apache configuration distiller, to ensure your includes remain after future cPanel updates:
    <p class="code_block" style="width: 500px;">/usr/local/cpanel/bin/apache_conf_distiller --update</p>
    <p>After running that you should get back:</p>
    <p class="code_block" style="width: 500px;">
      info [apache_conf_distiller] 'local' datastore in use (/var/cpanel/conf/apache/local)<br />
...
<ol class="article_list">
<li>Run the following command to create the directory where we'll be placing our ModSecurity configuration file:
<p class="code_block" style="width: 500px;">mkdir -p /usr/local/apache/conf/userdata/std/2/userna5/example.com/</p>
</li>
<li>Next use the following rule to create a ModSecurity configuration file with the <strong>SecRuleEngine</strong> option set to <strong>Off</strong>&nbsp;which disables ModSecurity completely for the domain:
<p class="code_block" style="width: 500px;">echo "SecRuleEngine Off" &gt; /usr/local/apache/conf/userdata/std/2/userna5/example.com/modsec.conf</p>
</li>
<li>Finally to make the new ModSecurity configuration file active, you need to restart the Apache service gracefully with the following command:
<p class="code_block" style="width: 500px;">service httpd graceful</p>
<p>Now that Apache has been gracefully restarted the new ModSecurity configuration settings should be getting loaded, and in this case ModSecurity is completely disabled for our <strong>example.com</strong> domain now.</p>
</li>
</ol>
<p>You should now understand how to completely disable ModSecurity for a domain on your VPS or dedicated server. If you wanted to leave ModSecurity enabled, but only wanted to disable certain ModSecurity rules that are getting triggered, then you can read my guide on how to <a href="/support/website/website-troubleshooting/find-and-disable-specific-modsecurity-rules" target="_blank">find and disable specific ModSecurity rules</a>.</p> 
...
<div style='display:inline-block;'>
	<iframe src='//www.facebook.com/plugins/like.php?href=http://www.inmotionhosting.com/support/website/modsecurity/disable-modsecurity-for-a-domain&amp;send=false&amp;layout=button_count&amp;width=100&amp;show_faces=false&amp;font=verdana&amp;colorscheme=light&amp;action=like&amp;height=21&amp;appId=209145459223160' scrolling='no' frameborder='0' style='border:none; overflow:hidden; width:100px; height:21px;' allowTransparency='true'></iframe>
</div>

	                        
<div style='display:inline-block;'>
	<div class='g-plusone' data-size='medium' data-href='http://www.inmotionhosting.com/support/website/modsecurity/disable-modsecurity-for-a-domain'></div>
	<script type='text/javascript'>
	  (function() {
	...
	<div name='sc_right' id='sc_right'>
		...
		<input type="hidden" name="return" value="aW5kZXgucGhwP29wdGlvbj1jb21fY29udGVudCZjYXRpZD00MDgmaWQ9NTY0JnZpZXc9YXJ0aWNsZSZJdGVtaWQ9MTAx" />
