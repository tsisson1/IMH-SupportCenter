<?php die("Access Denied"); ?>#x#a:3:{s:4:"body";s:72333:" 
...
<meta name="viewport" content="width=device-width" />
		  ...
		  <base href="http://www.inmotionhosting.com/support/website/server-usage/identify-and-block-bad-robots-from-website" />
  <meta http-equiv="content-type" content="text/html; charset=utf-8" />
  <meta http-equiv="last-modified" content="2015-07-17T14:55:56-07:00" />
  <meta name="keywords" content="identify find locate block stop bad bot bots robot robots from website .htaccess" />
  ...
  <link href="http://www.inmotionhosting.com/support/website/server-usage/identify-and-block-bad-robots-from-website" rel="canonical" />
...
<script type="text/javascript">
  ...
  (function() {
	...
	<!-- cid:f04348c12fb81e2a96a4491fd0a2229c -->
...
<script type="text/javascript">
                ...
                window.open('https://www.inmotionhosting.com/support/launch-chat/?rdid=2324923566971164486&url=' + escape(document.location.href), 'Chat8718689652968401074', 'toolbar=0,scrollbars=1,location=0,statusbar=0,menubar=0,resizable=1,width=640,height=650');
                        ...
                        _gaq.push(['_setAccount', 'UA-1501988-22']);
                        _gaq.push(['_trackEvent', 'chat-by-ip', 'launch-by-ip', '100.43.90.9']);
...
<div class='col_1'>
    <ul>
		...
		</li><li class='bc_arrow'>&gt;&gt;</li><li class='bc_no_dd' itemscope itemtype='http://data-vocabulary.org/Breadcrumb'><span itemprop='title'>Identify and block bad robots from website</span></li>			</ul>
		</div>
	</div>
</div>
...
<div class='container'>
	<div class='main-left-sc' name='sc_left' id='sc_left' style='max-width:705px;'>
		...
		<div class="page-header">
						<h1>
															Identify and block bad robots from website									</h1>
					...
					<div style='font-size:12px; color:#aaa; margin-bottom:-10px; padding-top:5px;'>
							...
							<div style='float:left; margin-right:20px;'><span style='color:#606060;'>Views:</span> <span name='page_views' id='page_views'>13,965</span></div>
...
<h2>Identify a bad robot</h2>
<p>Using the steps below, I'll show you some good steps to take in order to verify if a robot is a good or bad one.</p>
<p class="alert">Please note in order to follow these steps you would need to be on either a <a href="http://www.inmotionhosting.com/cloud-vs-vps">VPS</a> (Virtual Private Server) or dedicated server that has SSH access. If you're on a shared server you could read our guide on <a href="/support/website/cpanel/enable-raw-access-logs-in-cpanel" target="_blank">enabling raw access log archiving in cPanel</a> to be able to view the same data, but it would have to be on your own local computer.</p>
<ol class="article_list">
<li><a href="/support/website/general-server-setup/how-to-login-ssh" target="”_blank”">Login to your server via SSH</a>.</li>
<li>Navigate to your user's home directory where the Apache access logs are stored, in this case our username is <strong>userna5</strong>, so we'll use the following command:
<p class="cli" style="width: 500px; white-space: nowrap;">cd ~userna5/access-logs/</p>
</li>
<li>We can now use the following command to see all <strong>User-Agents</strong> that have requests to our <strong>example.com</strong>&nbsp;website:
<p class="cli" style="width: 500px; white-space: nowrap;">cat example.com | awk -F\" '{print $6}' | sort | uniq -c | sort -n</p>
<p>This gives us back the output:</p>
<p class="cli" style="width: 500px; white-space: nowrap;">638 Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)<br /> 1015 msnbot-UDiscovery/2.0b (+http://search.msn.com/msnbot.htm)<br /> 1344 Mozilla/5.0 (en-US) AppleWebKit/537.4 (KHTML, like Gecko) Chrome/22.0.1229 Safari/537.4 pss-webkit-request<br /> 21937 -</p>
<p>So in this case, there have been (21,937) requests that aren't supplying a <strong>User-Agent</strong> string. This is an immediate <span style="color: red;">red</span> flag, as typically any human visitor requesting a page from your website should have the <strong>User-Agent</strong> of their web-browser in each request.</p>
<p>We can also see that the next highest level of requests came from something calling itself <strong>pss-webkit-request</strong>, followed by <strong>msnbot-UDiscovery/2.0b</strong>, and then <strong>Googlebot/2.1</strong></p>
</li>
<li>First we can see all of the requests that didn't provide a <strong>User-Agent</strong>&nbsp;string, and then view all of the unique IP addresses that sent those requests in, with the following code:
<p class="cli" style="width: 500px; white-space: nowrap;">cat example.com | awk -F\" '$6 ~ "-"' | awk '{print $1}' | sort -n | uniq -c | sort -n</p>
<p>In this example here are the top IPs that had requests without a <strong>User-Agent</strong> string:</p>
<p class="cli" style="width: 500px; white-space: nowrap;">421 74.125.176.94<br /> 434 74.125.176.85<br /> 463 74.125.176.95</p>
</li>
<li>We can now search for these IP address against our access log to see what might going on with their requests. The following command is going to look for what <strong>User-Agent</strong> strings are coming from the one <strong>74.125.176.95</strong>&nbsp;IP address which had the most requests:
<p class="cli" style="width: 500px; white-space: nowrap;">grep 74.125.176.95 example.com | awk -F\" '{print $6}' | sort | uniq -c | sort -n</p>
<p>This gives us back:</p>
<p class="cli" style="width: 500px; white-space: nowrap;">7 Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.17 (KHTML, like Gecko) Chrome/24.0.1312.57 Safari/537.17<br /> 11 Mozilla/5.0 (Macintosh; Intel Mac OS X 10_7_5) AppleWebKit/537.17 (KHTML, like Gecko) Chrome/24.0.1312.57 Safari/537.17<br /> 29 Mozilla/5.0 (en-US) AppleWebKit/537.4 (KHTML, like Gecko) Chrome/22.0.1229 Safari/537.4 pss-webkit-request<br /> 434 -</p>
<p>Again this is another <span style="color: red;">red</span> flag, typically requests coming from a single IP address will use the same <strong>User-Agent</strong> string for each request. We can go a step further and run the following command to find out some information about the IP address:</p>
<p class="cli" style="width: 500px; white-space: nowrap;">whois 74.125.176.95</p>
<p>Some of the pertinent information that command gives us back for that IP is:</p>
<p class="cli" style="width: 500px; white-space: nowrap;">NetRange: 74.125.0.0 - 74.125.255.255<br /> NetName: GOOGLE<br /> OrgName: Google Inc.<br /> OrgId: GOGL<br /> Address: 1600 Amphitheatre Parkway<br /> City: Mountain View<br /> StateProv: CA<br /> PostalCode: 94043<br /> Country: US<br /> RegDate: 2000-03-30<br /> Updated: 2011-09-24<br /> Ref: http://whois.arin.net/rest/org/GOGL</p>
<p>So this is an IP address that belongs to Google, and so are all the other IPs we saw coming from the <strong>74.125</strong> IP range. However this isn't coming from the official Googlebot crawler which would be identified as such by the <strong>User-Agent</strong> string, but instead these are requests from their AppEngine/Cloud service.</p>
<p>So these requests are from custom crawlers more than likely that other users have made, and in some cases they could simply be trying to index your content for their own purposes instead of providing links back to you, which would fall under our definition of a bad robot.</p>
</li>
</ol>
<h2>Block a bad robot</h2>
...
<ol class="article_list">
<li>Edit the <strong>.htaccess</strong>&nbsp;file for your website with the following command:
<p class="cli" style="width: 500px; white-space: nowrap;">vim ~userna5/public_html/.htaccess</p>
<p>Once the <strong>vim</strong> text-editor has loaded the file, hit <strong>i</strong> to enter <strong>Insert</strong> mode, enter in the following code (in most SSH clients you can also right-click to paste text from the clipboard):</p>
<p class="cli" style="width: 500px; white-space: nowrap;">ErrorDocument 503 "Site disabled for crawling"<br /> RewriteEngine On<br /> RewriteCond %{HTTP_USER_AGENT} !^.*(Google).*$<br /> RewriteCond %{REMOTE_ADDR} ^74.125<br /> RewriteRule .* - [R=503,L]</p>
<p>Once you've entered that information in, hit <strong>Esc</strong> to exit <strong>Insert</strong> mode, then hold down <strong>Shift</strong> and type in <strong>ZZ</strong> to save the file.</p>
<p>These rules will first check if the <strong>User-Agent</strong> has the word <strong>Google</strong> in it anywhere, if it does not, it moves onto the next rule, which checks to see if the IP address begins with <strong>74.125</strong>. If the IP matches then it will serve them a <strong>503</strong> response of <strong>Site disabled for crawling</strong> which uses up very minimal server resources, instead of allowing them to hit your various website pages that could be causing a lot of usage.</p>
</li>
<li>As a bonus you can then use the following command to check up on how many requests you're saving your server from having to serve to these bad bots with this command:
<p class="cli" style="width: 500px; white-space: nowrap;">cat ~userna5/access-logs/example.com | grep "74.125" | awk '$9 ~ 503' | cut -d[ -f2 | cut -d] -f1 | awk -F: '{print $2":00"}' | sort -n | uniq -c | sed 's/[ ]*//'</p>
<p>This gives you back the amount of bots blocked per hour with your rule:</p>
<p class="cli" style="width: 500px; white-space: nowrap;">2637 14:00<br /> 2823 15:00<br /> 2185 16:00</p>
</li>
</ol>
...
<div style='display:inline-block;'>
	<iframe src='//www.facebook.com/plugins/like.php?href=http://www.inmotionhosting.com/support/website/server-usage/identify-and-block-bad-robots-from-website&amp;send=false&amp;layout=button_count&amp;width=100&amp;show_faces=false&amp;font=verdana&amp;colorscheme=light&amp;action=like&amp;height=21&amp;appId=209145459223160' scrolling='no' frameborder='0' style='border:none; overflow:hidden; width:100px; height:21px;' allowTransparency='true'></iframe>
</div>

	                        
<div style='display:inline-block;'>
	<div class='g-plusone' data-size='medium' data-href='http://www.inmotionhosting.com/support/website/server-usage/identify-and-block-bad-robots-from-website'></div>
	<script type='text/javascript'>
	  (function() {
	...
	<div name='sc_right' id='sc_right'>
		...
		<input type="hidden" name="return" value="aW5kZXgucGhwP29wdGlvbj1jb21fY29udGVudCZjYXRpZD04NSZpZD01ODMmdmlldz1hcnRpY2xlJkl0ZW1pZD0xMDE=" />
...
<div style="text-align: right; visibility: hidden; "><a style="text-decoration:none; color: #c0c0c0; font-family: arial,helvetica,sans-serif; font-size: 5pt;" target="_blank" href="http://joomline.org/extensions/component-for-joomla/slogin.html">Social Login Joomla</a></div>		</div>
			<div class="moduletable">
				...
				<div style='margin:10px 0px;'>
                                        <a href='/support/community-support/server-usage/googlebot-cant-access-my-site?tsrc=rsbraq'> Googlebot can't access my site</a>
                                </div>
			
				<div style='margin:10px 0px;'>
                                        <a href='/support/community-support//?tsrc=rsbraq'>What is this &quot;NEW&quot; user agent?</a>
                                </div>
			...
			<table style='width:100%; border-collapse:separate; border:1px solid #aaa;' class='sc_rc'>
				<tr>
					<td valign='top' style='width:100px; padding:5px; border-right:1px solid #aaa;background-image: -webkit-linear-gradient(top,#e3e3e3,#efefef);background-image: linear-gradient(to bottom, #E3E3E3, #EFEFEF); border-bottom-left-radius: 5px; border-top-left-radius: 5px;'>
						<img src='//www.gravatar.com/avatar/a90905357503c43a268febcd671f7673?s=90' class='sc_rc' style='max-width:90px; margin-right:15px;display:block;' />
						<div><a style='font-size:16px;' href='//www.gravatar.com/a90905357503c43a268febcd671f7673'>Chuck Gelm</a></div>
						<div></div>
						<div></div>
					...
					<td valign='top' style='padding:5px; background:#fff; border-bottom-right-radius: 5px; border-top-right-radius: 5px;'>
						<div style='float:right;margin:0px 0px 15px 15px;font-size:12px;color:#aaa'>2014-05-14 7:47 am</div>
				                <div><p>I use PHP and filter on IP address and User-Agent to make the 'bad bot' wait 999 seconds</p>
<p>and return 0 bytes.</p>
<p>http://gelm.net/How-to-block-Baidu-with-PHP.htm</p></div>
						<div style='text-align:right; clear:both;'><a href='#post_a_comment' class='btn btn-primary' style='text-transform:none;' onClick="jQuery('#new_comment_parent_id').val(10627);jQuery('html, body').animate({scrollTop: jQuery('#main_comment_submission_div').offset().top}, 1500);return false;" >Reply</a></div>
		...
		<a name='comment-29152'></a>
		<div  style='margin:40px 0px 0px 0px;'>
			
			<table style='width:100%; border-collapse:separate; border:1px solid #aaa;' class='sc_rc'>
				<tr>
					<td valign='top' style='width:100px; padding:5px; border-right:1px solid #aaa;background-image: -webkit-linear-gradient(top,#e3e3e3,#efefef);background-image: linear-gradient(to bottom, #E3E3E3, #EFEFEF); border-bottom-left-radius: 5px; border-top-left-radius: 5px;'>
						<img src='//www.gravatar.com/avatar/d9c7f06382936440ff367588d9384328?s=90' class='sc_rc' style='max-width:90px; margin-right:15px;display:block;' />
						<div><a style='font-size:16px;' href='//www.gravatar.com/d9c7f06382936440ff367588d9384328'>Jason Buzzeo</a></div>
						<div></div>
						<div></div>
						<div><span title='Posted anonymously - anonymous users receive no points'>n/a <a href='/support/team/points' target='_blank'>Points</a></span></div>
					...
					<td valign='top' style='padding:5px; background:#fff; border-bottom-right-radius: 5px; border-top-right-radius: 5px;'>
						<div style='float:right;margin:0px 0px 15px 15px;font-size:12px;color:#aaa'>2015-12-13 5:56 am</div>
				                <div><p>Thanks for this, what a great way to handle bad bots!</p>
<p>We've been harvesting the IPs from a contact form on our site.&nbsp; The form has multiple required fields, but bots are able to crawl the <strong>form action</strong> url without invoking the field validator, so the result is an e-mail with no content other than the senders IP address.</p>
<p>We store the IPs in a flat file on the server and then redirect any traffic coming from that address to www.***.gov/default.aspx. Our solutions works well, however, your solution should catch more "intruders" and reject them in a more efficient way.</p></div>
						<div style='text-align:right; clear:both;'><a href='#post_a_comment' class='btn btn-primary' style='text-transform:none;' onClick="jQuery('#new_comment_parent_id').val(29152);jQuery('html, body').animate({scrollTop: jQuery('#main_comment_submission_div').offset().top}, 1500);return false;" >Reply</a></div>
