<?php die("Access Denied"); ?>#x#a:3:{s:4:"body";s:64112:" 
...
<meta name="viewport" content="width=device-width" />
		  ...
		  <base href="http://www.inmotionhosting.com/support/email/mailing-lists/rate-limit-phplist" />
  <meta http-equiv="content-type" content="text/html; charset=utf-8" />
  <meta http-equiv="last-modified" content="2012-01-04T19:59:33-08:00" />
  <meta name="keywords" content="250 batch mailqueue_batch_size mailqueue_batch_period phplist send email rate limit" />
  ...
  <link href="http://www.inmotionhosting.com/support/email/mailing-lists/rate-limit-phplist" rel="canonical" />
...
<script type="text/javascript">
  ...
  (function() {
	...
	<!-- cid:76c299572636f962d70fa7a1271050b4 -->
...
<script type="text/javascript">
                ...
                window.open('https://www.inmotionhosting.com/support/launch-chat/?rdid=2324923566971164486&url=' + escape(document.location.href), 'Chat8718689652968401074', 'toolbar=0,scrollbars=1,location=0,statusbar=0,menubar=0,resizable=1,width=640,height=650');
                        ...
                        _gaq.push(['_setAccount', 'UA-1501988-19']);
                        _gaq.push(['_trackEvent', 'chat-by-ip', 'launch-by-ip', '54.85.191.18']);
...
<div class='container'>
	<div class='main-left-sc' name='sc_left' id='sc_left' style='max-width:705px;'>
		...
		<div class="page-header">
						<h1>
															How to rate limit emails sent in phpList									</h1>
					...
					<div style='font-size:12px; color:#aaa; margin-bottom:-10px; padding-top:5px;'>
							...
							<div style='float:left; margin-right:20px;'><span style='color:#606060;'>Views:</span> <span name='page_views' id='page_views'>10,083</span></div>
							<div style='float:left; margin-right:20px;' title='Originally published: Dec 21, 2011'><span style='color:#606060;'>Published:</span> Jan 4, 2012</div>
							...
							<div style='clear:both;'></div>
								...
								<p>In our previous article, we<a href="/support/email/mailing-lists/install-php-list"> installed phpList</a>. Before we continue with <a href="/support/email/mailing-lists/create-mailing-list-in-phplist">creating a mailing list that users can subscribe to</a>, there are some default settings that we must change within the phpList configuration.</p>
...
<p>Please note that we do on a case by case basis increase this number for our users. If you're interested in being able to send more than 250 emails per hour per domain, please see our <a href="/support/email/mailing-lists/distribution-mailing-lists-bulk-mailing-information-request">Bulk Mailing Request</a> article.</p>
...
<li>You are now looking at the phpList configuration file. There is quite a bit of content in this file, your focus is on finding these two lines:<br />define("MAILQUEUE_BATCH_SIZE",0);<br />define("MAILQUEUE_BATCH_PERIOD",100);<br />Change these values to what we have below:<br />define("MAILQUEUE_BATCH_SIZE",17);<br />define("MAILQUEUE_BATCH_PERIOD",300);<br />The changes that we made told phpList to send 17 emails every 300 seconds (5 minutes).</li>
...
<p># batch processing<br /># if you are on a shared host, it will probably be appreciated if you don't send<br /># out loads of emails in one go. To do this, you can configure batch processing.<br /># Please note, the following two values can be overridden by your ISP by using<br /># a server wide configuration. So if you notice these values to be different<br /># in reality, that may be the case</p>
<p>## if you send the queue using your browser, you may want to consider settings like this<br />## which will send 10 messages and then reload the browser to send the next 10. However, this<br />## will not restrict the sending to any limits, so there's a good chance you will<br />## go over the limits of your ISP<br /><em><span style="color: #ff0000;"># define("MAILQUEUE_BATCH_SIZE",10);</span></em><br /><em><span style="color: #ff0000;"># define("MAILQUEUE_BATCH_PERIOD",1);</span></em></p>
<p>## if you send the queue using commandline, you can set it to something that complies with the<br />## limits of your ISP, eg 300 messages an hour would be <br /><span style="color: #ff0000;"><em># define("MAILQUEUE_BATCH_SIZE",300);</em></span><br /><span style="color: #ff0000;"><em># define("MAILQUEUE_BATCH_PERIOD",3600);</em></span><br /># and then you need to set the cron to run every 5 minutes</p>
<p># define the amount of emails you want to send per period. If 0, batch processing<br /># is disabled and messages are sent out as fast as possible<br /><span style="color: #008000;"><strong>define("MAILQUEUE_BATCH_SIZE",17);</strong></span></p>
<p># define the length of one batch processing period, in seconds (3600 is an hour)<br /># Please note: this setting has two consequences:<br /># 1. it will enforce that the amount of emails sent in the period identified here does not exceed the amount<br />#&nbsp; set in MAILQUEUE_BATCH_SIZE<br /># 2. there will be a delay of MAILQUEUE_BATCH_PERIOD when running the queue.<br />#<br /># number 1 is mostly when using commandline queue processing (strongly recommended)<br /># number 2 is when using browser queue processing. The browser will reload to send the next<br /># batch after the amount of seconds set here</p>
...
<p>Later in our tutorial series, we'll show you <a href="/support/email/mailing-lists/how-to-send-a-message-to-your-phplist-mailing-list">how to send a message to a mailing list in phpList</a>. Because we're talking about rate limiting right now, we'd like to give you a sneak peak at what it will look like when you send to 400+ users with the above options set.</p>
...
<div class="code_block">Started<br /> Sending in batches of 17 emails<br /> Processing has started, 1 message(s) to process.<br /> Please leave this window open. You have batch processing enabled, so it will reload several times to send the messages. Reports will be sent by email to listreports@phplist.inmotiontesting.com<br /> Processing message 7<br /> Looking for users<br /> Found them: 403 to process<br /> batch limit reached: 17 (17)<br /> Script stage: 5<br /> 17 messages sent in 4.38 seconds (13961 msgs/hr)<br /> Finished this run<br /> Waiting for 300 seconds before reloading</div>
...
<div class="code_block">Started<br /> Sending in batches of 17 emails<br /> Sent in last run: 17<br /> Skipped in last run: 0<br /> Processing has started, 1 message(s) to process.<br /> Please leave this window open. You have batch processing enabled, so it will reload several times to send the messages. Reports will be sent by email to listreports@phplist.inmotiontesting.com<br /> Processing message 7<br /> Looking for users<br /> Found them: 386 to process<br /> batch limit reached: 17 (17)<br /> Script stage: 5<br /> 17 messages sent in 0.51 seconds (119386 msgs/hr)<br /> Finished this run<br /> Waiting for 300 seconds before reloading</div>
...
<p>To continue sending your list, simply log back into you phpList admin and click the "process queue" link in the right sidebar:<br /><a class="std_ss" href="//cdn.inmotionhosting.com/support/images/stories/phplist/in-phplist-click-process-queue.gif"><img  alt="in-phplist-click-process-queue" src="//cdn.inmotionhosting.com/support/images/stories/phplist/in-phplist-click-process-queue.gif" /></a></p> 
...
<div style='display:inline-block;'>
	<div class='g-plusone' data-size='medium' data-href='http://www.inmotionhosting.com/support/email/mailing-lists/rate-limit-phplist'></div>
	<script type='text/javascript'>
	  (function() {
	...
	<div name='sc_right' id='sc_right'>
		...
		<input type="hidden" name="return" value="aW5kZXgucGhwP29wdGlvbj1jb21fY29udGVudCZjYXRpZD0yNCZpZD03Njgmdmlldz1hcnRpY2xlJkl0ZW1pZD0xMDE=" />
		<input type="hidden" name="50773dabf63ebe72fe547c22101e801a" value="1" />	</div>
	</form>
<!-- <br />
			...
			<table style='width:100%; border-collapse:separate; border:1px solid #aaa;' class='sc_rc'>
				<tr>
					<td valign='top' style='width:100px; padding:5px; border-right:1px solid #aaa;background-image: -webkit-linear-gradient(top,#e3e3e3,#efefef);background-image: linear-gradient(to bottom, #E3E3E3, #EFEFEF); border-bottom-left-radius: 5px; border-top-left-radius: 5px;'>
						<img src='//www.gravatar.com/avatar/48c9c394b8d7bfa6ae621ed4932a3d4b?s=90' class='sc_rc' style='max-width:90px; margin-right:15px;display:block;' />
						<div><a style='font-size:16px;' href='//www.gravatar.com/48c9c394b8d7bfa6ae621ed4932a3d4b'>Alex</a></div>
						<div></div>
						<div></div>
					...
					<td valign='top' style='padding:5px; background:#fff; border-bottom-right-radius: 5px; border-top-right-radius: 5px;'>
						<div style='float:right;margin:0px 0px 15px 15px;font-size:12px;color:#aaa'>2016-03-17 6:46 pm</div>
				                <div><p>How could I imagine that I should limit the sending emails if there is no configuration or advice inside the PHPList / Softaculous software??My account was suspended and it looks like I lost access to all my work, all my websites and cannot even make a backup of them!Softaculous should do this! Or, give a notice about it!</p></div>
						<div style='text-align:right; clear:both;'><a href='#post_a_comment' class='btn btn-primary' style='text-transform:none;' onClick="jQuery('#new_comment_parent_id').val(31305);jQuery('html, body').animate({scrollTop: jQuery('#main_comment_submission_div').offset().top}, 1500);return false;" >Reply</a></div>
		...
		<div  style='margin:2px 0px 0px 40px;'>
			...
			<table style='width:100%; border-collapse:separate; border:1px solid #aaa;' class='sc_rc'>
				<tr>
					...
					<td valign='top' style='padding:5px; background:#fff; border-bottom-right-radius: 5px; border-top-right-radius: 5px;'>
						<div style='float:right;margin:0px 0px 15px 15px;font-size:12px;color:#aaa'>2016-03-18 7:13 am</div>
				                <div>The hourly limit for each cPanel account is set within the server itself at the admin level. PHPList and Softaculous have no way of seeing this limit. This can be changed by requesting an exemption within your AMP. If you upgrade to a VPS or Dedicated plan, there is no limit to sending emails.</div>
						<div style='text-align:right; clear:both;'><a href='#post_a_comment' class='btn btn-primary' style='text-transform:none;' onClick="jQuery('#new_comment_parent_id').val(31311);jQuery('html, body').animate({scrollTop: jQuery('#main_comment_submission_div').offset().top}, 1500);return false;" >Reply</a></div>
