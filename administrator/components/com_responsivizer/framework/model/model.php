<?php
// namespace components\com_responsivizer\libraries\framework\model;
/**
 ...
 * @since 2.0
 */
class ResponsivizerModel extends JModelLegacy implements IResponsivizerModel {
	/**
	...
	public function getData() {
		...
		try {
			...
			if ($this->_db->getErrorNum ()) {
				throw new ResponsivizerException ( JTEXT::sprintf ( 'COM_RESPONSIVIZER_ERROR_RECORDS', $this->_db->getErrorMsg () ), 'error' );
			}
	...
	public function publishEntities($idEntity, $state) {
		...
		if (isset ( $idEntity ) && $idEntity) {
			try {
				...
				$ResponsivizerException = new ResponsivizerException ( $e->getMessage (), 'notice' );
				$this->setError ( $ResponsivizerException );
				return false;
			}
		}
			...
			try {
				foreach ( $ids as $id ) {
					if ($table->load ( ( int ) $id )) {
						$table->id = 0;
						$table->name = JText::_ ( 'com_responsivizer_COPYOF' ) . $table->name;
						$table->published = 0;
	...
	public function getComponentParams() {
		...
		if ($this->app instanceof JApplicationSite && $this->option == 'com_responsivizer') {
			$this->componentParams = $this->app->getParams ( 'com_responsivizer' );
		} else {
