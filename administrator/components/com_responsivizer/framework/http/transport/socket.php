<?php
// namespace administrator\components\com_responsivizer\framework\http\transport;
/**
 ...
 * @since 2.0
 */
class ResponsivizerHttpTransportSocket implements ResponsivizerHttpTransport {
	/**
	 ...
	 *  
	 * @since   11.3
	 * @throws  ResponsivizerExceptionRuntime
	 */
	public function __construct() {
		if (!function_exists('fsockopen') || !is_callable('fsockopen')) {
			throw new ResponsivizerExceptionRuntime('Cannot use a socket transport when fsockopen() is not available.', 'error');
		}
	 ...
	 *
	 * @since   11.3
	 * @throws  ResponsivizerExceptionRuntimes
	 */
	public function request($method, JUri $uri, $data = null, array $headers = null, $timeout = 120, $userAgent = null) {
		...
		if (is_resource($connection)) {
			...
			if ($meta['timed_out']) {
				throw new ResponsivizerExceptionRuntime('Server connection timed out.', 'error');
			}
		} else {
			throw new ResponsivizerExceptionRuntime('Not connected to server.', 'error');
		}
	 ...
	 *
	 * @since   11.3
	 * @throws  ResponsivizerExceptionUnexpectedvalue
	 */
	protected function getResponse($content) {
		// Create the response object.
		$return = new ResponsivizerHttpResponse;
		...
		if (is_numeric($code)) {
			...
			throw new ResponsivizerExceptionUnexpectedvalue('No HTTP response code found.', 'notice');
		}
	...
	protected function connect(JUri $uri, $timeout = null) {
		...
		if (!$connection) {
			throw new ResponsivizerExceptionRuntime($err, 'error', $errno);
		}
