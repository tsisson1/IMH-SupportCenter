...
if( $_POST['pcm_task'] == "change_message" )
{
	...
	$query = "UPDATE #__prechat_message SET `message` = '" . addslashes($_POST['prechat_message']) . "', `last_update` = CURRENT_TIMESTAMP , `expiration` = '$expire_date'  WHERE `id` = 5;";
	$db->setQuery($query);
	$result = $db->execute();


	$email_body = "MESSAGE:\n" . $_POST['prechat_message'] . "\n\nEXPIRATION: " . $_POST['expiration'] . " hours";
	cc_send_mail("anthonye@inmotionhosting.com","cs-lead@inmotionhosting.com","IMH Prechat message EDITED",$email_body);
...
$query = "SELECT * FROM #__prechat_message WHERE `id` = 5;";
$db->setQuery($query);
