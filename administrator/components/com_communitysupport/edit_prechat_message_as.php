...
if( $_POST['pcm_task'] == "change_message" )
{
	...
	$query = "UPDATE #__prechat_message SET `message` = '" . addslashes($_POST['prechat_message']) . "', `last_update` = CURRENT_TIMESTAMP , `expiration` = '$expire_date'  WHERE `id` = 7;";
	$db->setQuery($query);
	$result = $db->execute();
...
$query = "SELECT * FROM #__prechat_message WHERE `id` = 7;";
$db->setQuery($query);
