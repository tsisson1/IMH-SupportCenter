...
{
	$query = "UPDATE #__prechat_message SET `message` = '" . addslashes($_POST['prechat_message']) . "', `last_update` = CURRENT_TIMESTAMP WHERE `id` = 4;";
	$db->setQuery($query);
	$result = $db->execute();
...
$query = "SELECT * FROM #__prechat_message WHERE `id` = 4;";
$db->setQuery($query);
$results = $db->loadObjectList();
