...
*/}
if( $_GET['rdid'] == "advanced-support")
{
    unset($departments);
    $departments["7089180248416250951"] = "Advanced Support";
	...
	while($row = mysqli_fetch_array($result))
	{
		switch($row['id'])
		{
            ...
            case "7":
                $advanced_support_message_last_update = $row['last_update'];
                    $advanced_support_message_expiration = $row['expiration'];
                    $advanced_support_message = $row['message'];
            case "8":
                $advanced_support_default_message = $row['message'];
...
if( $billing_message_expiration != "0" )
    {
            $current_time = date("Y-m-d H:i:s");
            $billing_message_last_updated_seconds_ago = strtotime($current_time) - strtotime($billing_message_update);

        // if updated 1 hour 1 second ago is greate than 1 hour expiration
     if( $billing_message_last_updated_seconds_ago > ($billing_message_expiration * 60 * 60) )
        {
            $query = "UPDATE `" . $cg->dbprefix . "prechat_message` SET `message` = '" . addslashes($billing_default_message) . "', `last_update` = CURRENT_TIMESTAMP, `expiration` = 0 WHERE `id` = 5;";
            $result = mysqli_query($con,$query);
...
if( $advanced_support_message_expiration != "0" )
    {
            $current_time = date("Y-m-d H:i:s");
            $advanced_support_message_last_updated_seconds_ago = strtotime($current_time) - strtotime($advanced_support_message_last_update);

         // if updated 1 hour 1 second ago is greate than 1 hour expiration
        // if 3601 seconds is greater than 3600
        if( $advanced_support_message_last_updated_seconds_ago > ($advanced_support_message_expiration * 60 * 60) )
        {
            $query = "UPDATE `" . $cg->dbprefix . "prechat_message` SET `message` = '" . addslashes($advanced_support_default_message) . "', `last_update` = CURRENT_TIMESTAMP, `expiration` = 0 WHERE `id` = 7;";
            $result = mysqli_query($con,$query);
            $advanced_support_message = $advanced_support_default_message;
        }
...
<script src="main.2.js?v=<? echo time(); ?>" type="text/javascript"></script>
...
<div name="ASMessage" id="ASMessage">
        <? echo $advanced_support_message; ?>
</div>
...
$('#launchSup').show();
    ...
    } else if ( $('#rdid').val() == '7089180248416250951' ){
        $('#lauchSup').hide();  
        $('#lauchReg').hide();  
        $('#launchCS').hide();
        $('#SalesMessage').hide();
...
<script>
$(document).ready( function() {
   // If the current url contains the string 'advanced-support'
   if (document.location.href.indexOf('advanced-support') > -1)
   {
      // Use jQuery to set the value of the select box to 'Advanced Support'
      $('#rdid').val('7089180248416250951');
}
});
</script>
<script>
$(document).ready( function() {
   // If the current url contains the string 'sales'
...
$('#rdid').bind('change', function (e) { 
    if(( $('#rdid').val() == '2324923566971164486') || ($('#rdid').val() == '1621843185299383618') || ($('#rdid').val() == '7089180248416250951'){
