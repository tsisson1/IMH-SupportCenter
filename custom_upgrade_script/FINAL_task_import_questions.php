...
$query = "DELETE FROM `$new_db_name`.`" . $new_db_fix . "question_to_question`;";
echo "<h1>Deleting all question to questions... (we'll import them again)</h1>";
mysql_query($query);
echo "<h2>done.</h2>";

$query = "DELETE FROM `$new_db_name`.`" . $new_db_fix . "question_updates`;";
echo "<h1>Deleting all question updates... (we'll import them again)</h1>";
mysql_query($query);
echo "<h2>done.</h2>";

$query = "DELETE FROM `$new_db_name`.`" . $new_db_fix . "answer_comments`;";
echo "<h1>Deleting all answer comments... (we'll import them again)</h1>";
mysql_query($query);
echo "<h2>done.</h2>";
...
while($row = mysql_fetch_array($result))
{
	...
	$description = html_entity_decode($row['details'],ENT_QUOTES);
