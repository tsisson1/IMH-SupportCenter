...
if (JRequest::getVar('option') == "com_content" AND JRequest::getVar('view') == "article" AND JRequest::getVar('id') == 3283) #AND $user->authorise('core.admin') == false  ) /* don't show comments on "join the community!" page */
{
	// don't show comments
}
else
...
function display_individual_comment($v,$total_comments,$count,$level)
{
    ...
    {   
      ...
      if( $_POST['approval_status'] == "SPAM Comment" )
        {   
            $query = "DELETE FROM #__comments WHERE `id` = " . $v->id . " LIMIT 1;";
            $db->setQuery($query);
                    $result = $db->query();
            $query = "INSERT INTO #__admintools_ipblock (`ip`,`description`) VALUES ('" . $v->ip . "', 'Spam Comment')";
            $db->setQuery($query);
                    $result = $db->query();
        }
         if( $_POST['approval_status'] == "APPROVE Comment" )
       {   
           $query = "UPDATE #__comments SET `hide_comment` = 0 WHERE `id` = " . $v->id . " LIMIT 1;";
           $db->setQuery($query);
                   $result = $db->query();
       }
#  if( $_POST['approval_status'] == "DELETE Comment" )
#        {   
#            $query = "DELETE FROM #__comments WHERE `id` = '" . $v->id . "' LIMIT 1;";
#var_dump($query);        
#    $db->setQuery($query);
	...
	if( $current_user->groups[8] == 8 )
	{
		...
		if($v->hide_comment == 1)
			$staff_edit_this_comment .= "	 <form method='post' name='approve_comment_" . $v->id . "' id='approve_comment_" . $v->id . "' style='font-size:10px;'>
						...
						<a href='' onClick=\"document.getElementById('approve_comment_" . $v->id . "').submit(); return false;\" style='color:green;'>Approve</a>
                                            </form>
<br> 
 <!--                  <form method='post' id='approval_status' style='margin:30px 0px;  padding:4px;'>
               <input type='hidden' name='comment_id' id='comment_id' value='" . $v->id . "' />
               <input type='submit' name='approval_status' id='approval_status' value='SPAM Comment' style='background:red;' />
               <input type='submit' name='approval_status' id='approval_status' value='APPROVE Comment' style='background:green;' />
                <input type='submit' name='approval_status' id='approval_status' value='DELETE Comment' style='background:red;' />  -->

           </form>
Comment Needs to be approved or deleted.
";
    }
...
';
	...
	if( $count == 1 AND $level == 0 AND "enabled" == "disabled")
	{
		echo "
	...
	if ($count == 1 AND $level == 0)
		$comment_container_div_style = "margin:0px;";
	else
...
{
	$ssl_subdomain = return_cdn_subdomain();
	...
	if (isset($GLOBALS['post_this_anonymous_comment']))
{	if( $GLOBALS['post_this_anonymous_comment'] == "no" )
		{
			...
			$acomment_bcomment = htmlentities($_POST['acomment_bcomment'],ENT_QUOTES);
			$acomment_phone = htmlentities($_POST['acomment_phone'],ENT_QUOTES);
		...
		<form method='post'>
			<table class='table acomment'>
				...
				<tr style='display:none;'>
					<th>Phone Number:</th>
					<td>
						<input type='text' name='acomment_phone' id='acomment_phone' style='width:97%;' value='$acomment_phone' />
					</td>
				</tr>
				<tr>
					<th>Comment:</th>
					<td>
						...
						<script type='text/javascript' src='//$ssl_subdomain/support/components/com_pup/tinymce/js/tinymce/tinymce.min.js'></script>
						...
						<textarea class='acomment_bcomment' name='acomment_bcomment' id='acomment_bcomment' style='display:none;'>$acomment_bcomment</textarea>
