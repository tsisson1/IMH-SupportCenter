...
function display_individual_comment($v,$total_comments,$count)
{
	...
	if( strlen($v->email_address) >= 5 )
		$author_avatar_url = "http://www.gravatar.com/avatar/" . md5(strtolower(trim($v->email_address))) . "?s=90";
	else
	...
	if( isset($_GET['new_comment']) OR "enabled" == "enabled")
	echo "
	        <div $border_style>
        	        $comment_header
			<table>
				<tr>
					<td valign='top' style='width:100px;'><img src='$author_avatar_url' class='sc_rc' style='max-width:90px; margin-right:15px;' /></td>
					<td style='width:550px;' valign='top'>
						...
						<div><a style='font-size:16px;' href='$url_to_profile'>$username_to_print</a> $community_support_technician</div>
