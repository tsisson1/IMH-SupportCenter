...
$bc_data['three_column_tools_and_resources'] = "
<div class='col_3'>
...
foreach($list as $k => $v)
{
	...
	if( $x == count($list) AND ($count_of_letters > 105) )
        ...
        if($k == 0)
		echo "<li class='home_item $bc_drop_class' itemscope itemtype='http://data-vocabulary.org/Breadcrumb'>";
        else
                echo "<li class='$bc_drop_class' itemscope itemtype='http://data-vocabulary.org/Breadcrumb'>";

        if($link)
                echo "<a href='$link' itemprop='url'><span itemprop='title'>$name<span></a>";
        else
...
function print_breadcrumb_drop($list,$k,$bc_data)
{
        switch($k)
        {
			...
			if($list[1]->link == "/support/edu")
				echo "<div class='dropdown_3columns'>" . $bc_data['one_column_pg'] .  $bc_data['one_column_kb'] . $bc_data['one_column_cs'] . $bc_data['three_column_tools_and_resources'] . "</div>";
			else
				echo "<div class='dropdown_3columns'>" . $bc_data['one_column_pg'] .  $bc_data['one_column_kb'] . $bc_data['one_column_cs'] . $bc_data['three_column_edu'] . $bc_data['three_column_tools_and_resources'] . "</div>";
			break;
		case 1:
			if($list[1]->link == "/support/edu")
			...
			elseif( $list[1]->link == "/support/tools" OR $list[1]->link == "/support/resources" )
				echo "<div class='dropdown_1column'>" . $bc_data['three_column_tools_and_resources'] . "</div>";
        ...
        elseif($list[1]->link == "/support/product-guides")
            {
                $drop_down = return_product_guides_listing_dropdown($k,$list[$k]->name,$list[$k]->link);
                echo $drop_down;
            }
...
function return_article_listing_dropdown($k,$name,$link)
{
	...
	$query = "SELECT `title`,`alias`,`hits` FROM #__content WHERE `catid` = $parent_id AND `state` = 1 ORDER BY `hits` DESC LIMIT 15;";
	$db->setQuery($query);
        $results = $db->loadObjectList();
	if( count($results) > 10 )
	{
...
function return_product_guides_listing_dropdown($k,$name,$link)
{   
    $db = JFactory::getDbo();
   ...
   $query = "SELECT `title`,`alias`,`hits` FROM #__content WHERE `catid` = $parent_id AND `state` = 1 ORDER BY `hits` DESC limit 5;";
    $db->setQuery($query);
        $results = $db->loadObjectList(); 
    if( count($results) > 10 )
    {   
