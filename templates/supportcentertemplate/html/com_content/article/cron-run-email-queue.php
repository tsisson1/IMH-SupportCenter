...
$query = "SELECT * FROM #__email_queue WHERE `been_sent` = 0 AND `sent_error_message` NOT LIKE '%invalid address%' ORDER BY `id` ASC LIMIT 10;";
$db->setQuery($query);
	...
	foreach ($email_queue as $key => $v)
	{
		try
		{
			...
			if ($body == "")
		        ...
		        $mail->Password   = "fJ;zKIJyCduC%&CM+P";            // SMTP server password
