...
abstract class JModuleHelper
{
	...
	protected static function &_load()
	{
		...
		$query->select('m.id, m.title, m.module, m.position, m.content, m.showtitle, m.params, mm.menuid, rm.mobile_exclude, rm.show_title, rm.show_bg');
		...
		// Added left join to exxlude modules mobile excluded by ids			
		$query->join('LEFT', '#__rvzr_modules AS rm ON rm.id = m.id');
		$query->where('(ISNULL(rm.mobile_exclude) OR rm.mobile_exclude = 0)');
		...
		for ($i = 0, $n = count($modules); $i < $n; $i++)
		{
			...
			// Manage force always sub menu items inject if module of type menu
			if($module->module == 'mod_menu') {
				$paramsObject = new JRegistry($module->params);
				$paramsObject->set('showAllChildren', 1);
				$module->params = $paramsObject->toString('JSON');
			}
