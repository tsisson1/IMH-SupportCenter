...
// echo "<pre>"; print_r($question_details); echo "</pre>";

// ####################12343234--md5(email)

$auto_unsubscribe_status = true;

$unable_to_unsubscribe_error_message = "We're sorry, but unfortunately there was a problem unsubscribing you from email alerts for this page.";
$able_to_unsubscribe_message = "You have successfully unsubscribed from email alerts for this page.";

// grab variables from url
$eu = explode("--",$_GET['unsubscribe_user']);
...
else
{
        ...
        if($email_address_from_database)
        {
                ...
                if ($md5_of_email_from_url == md5($email_address_from_database))
                {
                        ...
                        $query = "SELECT count(`id`) FROM #__questions_do_not_email_me WHERE `user_id` = $user_id_from_url AND `question_id` = " . $question_details[0]->id;
                        $db->setQuery($query);
                        $already_unsubscribed = $db->loadResult();

                        if($already_unsubscribed == 0)
                        {
                                $query = "INSERT INTO #__questions_do_not_email_me (`id`,`user_id`,`question_id`) VALUES(NULL,$user_id_from_url," . $question_details[0]->id . ");";
                                gf_run_query($query,$able_to_unsubscribe_message,$unable_to_unsubscribe_error_message);
                        }
                        else
                        {
                                JFactory::getApplication()->enqueueMessage("It appears you are already unsubscribed from email alerts for this page.", 'Success');
                        }
